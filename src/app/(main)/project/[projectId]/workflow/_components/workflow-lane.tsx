...
const WorkflowLane: React.FC<PipelaneLaneProps> = ({
  setAllTasks,
  tasks,
  workflowId,
  laneDetails,
  ...
  const laneAmt = useMemo(() => {
    console.log(tasks);
    return tasks.reduce((sum, task) => sum + (Number(task?.value) || 0), 0);
  }, [tasks]);
  ...
  const handleCreateTask = () => {
    setOpen(
      <CustomModal
        title="Create A Task"
        subheading="Tasks are a great way to keep track of tasks">
        <TaskForm
  ...
  const handleEditLane = () => {
    setOpen(
      <CustomModal title="Edit Lane Details" subheading="">
        <CreateLaneForm workflowId={workflowId} defaultData={laneDetails} />
      </CustomModal>
    );
  };
