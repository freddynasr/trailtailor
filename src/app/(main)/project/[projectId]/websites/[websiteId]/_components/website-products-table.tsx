...
interface WebsiteProductsTableProps {
  defaultData: Website;
  products: Stripe.Product[];
}

const WebsiteProductsTable: React.FC<WebsiteProductsTableProps> = ({
  products,
  defaultData,
}) => {
  ...
  const handleSaveProducts = async () => {
    setIsLoading(true);
    const response = await updateWebsiteProducts(
      JSON.stringify(liveProducts),
      defaultData.id
    );
    await saveActivityLogsNotification({
      agencyId: undefined,
      description: `Update website products | ${response.name}`,
...
export default WebsiteProductsTable;
